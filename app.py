...
with h_r:
    if not st.session_state.logged_in:
        ...
        if st.button("Pro 회원가입 신청"): st.session_state.show_reg = not st.session_state.show_reg
    else:
...
# [2] Material Specs (로그인 전 비활성화 및 텍스트 표시)
with st.container(border=True):
    st.markdown('<p class="main-header">2. Material Specs Expert Mode</p>', unsafe_allow_html=True)
    
    lock_suffix = " :red[(Pro Mode 전용)]" if not is_pro else ""
...
# [3] Process Parameters (로그인 전 비활성화 및 텍스트 표시)
with st.container(border=True):
    st.markdown('<p class="main-header">3. Process Parameters</p>', unsafe_allow_html=True)
    
    adv_lock_suffix = " :red[(Pro Mode 전용)]" if not is_pro else ""
...
with st.container(border=True):
    ...
    if st.session_state.sim_result:
        ...
        st.info("5번 섹션에서 시뮬레이션을 실행해 주세요.")

# [5] Simulation Control & History (버튼 및 로그 통합)
with st.container(border=True):
    ...
    if st.session_state.history:
        st.markdown("---")
        st.markdown('<p class="sub-header-bold">🔍 과거 기록 복원 및 로그</p>', unsafe_allow_html=True)
        log_opts = [f"[{h['Time']}] {h['Material']} | {h['Whkg']} Wh/kg" for h in st.session_state.history]
        sel_idx = st.selectbox("불러올 기록을 선택하세요.", range(len(log_opts)), format_func=lambda x: log_opts[x])
