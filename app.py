...
# -----------------------------------------------------------------------------
# [2] Material Specs (흐림 없음, 활성화 체크박스 자물쇠)
# -----------------------------------------------------------------------------
with st.container(border=True):
...
# -----------------------------------------------------------------------------
# [3] Process Parameters (흐림 없음, 활성화 체크박스 자물쇠)
# -----------------------------------------------------------------------------
with st.container(border=True):
    ...
    with p1: 
        ...
        if show_adv and is_pro: st.slider("Cathode Press Density", 1.5, 3.5, 2.5, key="c_press_m")
    with p2: 
        v_np = st.slider("N/P Ratio", 1.0, 1.5, 1.15, disabled=not is_pro)
        if show_adv and is_pro: st.slider("Anode Active Ratio %", 90.0, 98.0, 95.0, key="a_active_m")
    with p3: 
        v_act = st.slider("Active Ratio (%)", 80.0, 99.0, 92.0, disabled=not is_pro)
        if show_adv and is_pro: st.slider("Separator Thick (μm)", 12, 30, 16, key="sep_thick_m")

# -----------------------------------------------------------------------------
# [4] Analysis Result (독립된 분석 대시보드)
# -----------------------------------------------------------------------------
with st.container(border=True):
...
[Image of lithium-ion battery discharge curve]
        ...
        st.info("좌측 하단의 5번 섹션에서 설계를 실행하면 분석 결과가 여기에 표시됩니다.")
...
with st.container(border=True):
    ...
    if st.session_state.history:
        ...
        sel_idx = st.selectbox("기록을 선택하면 4번 분석 결과창에 데이터가 복원됩니다.", range(len(log_opts)), format_func=lambda x: log_opts[x])
        ...
        st.write("아직 시뮬레이션 이력이 없습니다. 위 버튼을 눌러 첫 설계를 시작하세요.")