...
with h_r:
    if not st.session_state.logged_in:
        ...
        if l_c3.button("Login", key="btn_login_m"):
            ...
            valid = df_u[(df_u['Email'].str.strip().str.lower() == u_id) & (df_u['Password'] == hashed_pw)] if not df_u.empty else pd.DataFrame()
            ...
            if st.button("가입신청", disabled=not (agree and pw1==pw2 and n_name), key="r_fin_m"):
                try:
                    ...
                    new_user = pd.DataFrame([{
                        ...
                        "Name": n_name, "Company": n_comp, "Dept": n_dept, "Job": n_job, "Phone": n_phone, 
...
with st.container(border=True):
    ...
    if expert:
        ...
        # 체크박스 해제(또는 잠금) 상태일 때는 텍스트로 깔끔하게 노출 (흐림 처리 아님)
...
# [4] Target & [5] Simulation History (초기 코드 그대로 유지)
with st.container(border=True):
    ...
    if st.button("🚀 RUN DESIGN SIMULATION", key="btn_run_m"):
        # 무료 시도 횟수 검사 로직을 삭제하고 무조건 실행되도록 수정
