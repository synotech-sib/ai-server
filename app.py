...
st.markdown("""
    ...
    /* 체크박스 옆 빨간색 Pro 전용 문구 (폰트 스타일 통일) */
    .pro-red-label {
        color: #ff0000;
        font-size: 1rem; /* Streamlit 기본 폰트 크기 */
        font-family: inherit;
        font-weight: 400;
        margin-left: -15px; /* 체크박스와의 간격 밀착 */
        padding-top: 5px;
    }
...
with h_r:
    if not st.session_state.logged_in:
        ...
        u_id = l_c1.text_input("ID", placeholder="email", key="id_login")
...
with st.container(border=True):
    ...
    # 체크박스와 빨간색 문구 나란히 배치
    ck_col1, ck_col2 = st.columns([0.25, 0.75])
    with ck_col1:
        expert = st.checkbox("🔓 물성 직접 수정 활성화", key="chk_exp", disabled=not st.session_state.logged_in)
    with ck_col2:
        if not st.session_state.logged_in:
            st.markdown('<p class="pro-red-label">(Pro Mode 전용)</p>', unsafe_allow_html=True)
    ...
    if expert and st.session_state.logged_in:
        ...
        s1.markdown(f'<p class="sub-header-bold">Capacity</p><p class="value-large">{v_cap} mAh/g</p>', unsafe_allow_html=True)
        s2.markdown(f'<p class="sub-header-bold">Voltage</p><p class="value-large">{v_volt} V</p>', unsafe_allow_html=True)
        s3.markdown(f'<p class="sub-header-bold">Density</p><p class="value-large">{v_dens} g/cc</p>', unsafe_allow_html=True)
        s4.markdown(f'<p class="sub-header-bold">Base Life</p><p class="value-large">{v_life:,} Cyc</p>', unsafe_allow_html=True)

# [3] Process Parameters
with st.container(border=True):
    st.markdown('<p class="main-header">3. Process Parameters</p>', unsafe_allow_html=True)
    
    ck_col3, ck_col4 = st.columns([0.35, 0.65])
    with ck_col3:
        show_adv = st.checkbox("🔍 상세 설정(Advanced) 활성화", key="chk_adv", disabled=not st.session_state.logged_in)
    with ck_col4:
        if not st.session_state.logged_in:
            st.markdown('<p class="pro-red-label">(Pro Mode 전용)</p>', unsafe_allow_html=True)
...
with st.container(border=True):
    ...
    if st.button("🚀 RUN DESIGN SIMULATION", use_container_width=True):
        ...
        log_entry = {
            ...
            "Loading": v_load, "NP": v_np, "Active": v_act
        }
    ...
    if st.session_state.history:
        ...
        with l_col1:
            ...
            sel_idx = st.selectbox("과거 시뮬레이션 기록 선택", range(len(log_opts)), format_func=lambda x: log_opts[x])
        with l_col2:
            st.markdown("<br>", unsafe_allow_html=True)
    ...
    if st.session_state.sim_result:
        ...
        st.markdown(f"### 📊 Result: {res['Wh/kg']} Wh/kg ({res['Time']})")
        r_c1, r_c2, r_c3 = st.columns(3)
        r_c1.metric("Energy", f"{res['Wh/kg']} Wh/kg")
        r_c2.metric("Voltage", f"{res['Volt']} V")
        r_c3.metric("Life", f"{res['Life']:,} Cyc")
