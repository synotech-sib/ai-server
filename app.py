...
def load_user_history(email, workspace="material_list"):
    ...
    try:
        ...
        if db_df.empty or 'Email' not in db_df.columns: 
            return []
...
# ✅ 상단 헤더에도 gap="large"를 적용하여 하단 본문 폭과 완벽히 일치하도록 조절
            ...
            if st.session_state.admin_view:
                ...
                try:
                    if st.session_state.admin_view == 'mats' and st.session_state.admin_ws == 'material_overall':
                        ...
                        if not tmp_public.empty:
                            dfs.append(tmp_public)
                        ...
                        if st.button("💾 변경사항 클라우드에 저장", type="primary"):
                            try:
                                ...
                                save_df = save_df[original_cols] if set(original_cols) == set(save_df.columns) else save_df
...
with col_left:
    ...
    try:
        ...
        # 없으면 텍스트(SC)로 대체합니다
        st.markdown("<div style='text-align: center; color: #bbb; font-weight: bold; margin-top: 10px; font-size: 20px; letter-spacing: 1px;'>SC</div>", unsafe_allow_html=True)
    ...
    with st.container(height=900, border=False):
            ...
            with c_1:
                if is_pro and st.session_state.workspace == "material_overall":
                    ...
                    for v in vips:
                        ...
                        if not tmp.empty:
                            dfs.append(tmp.assign(Is_VIP=True).iloc[::-1])
                    if not mat_df_public.empty:
                        dfs.append(mat_df_public.copy().assign(Is_VIP=False))
                    ...
                    _dfs = []
                    if not df_vip.empty:
                        _dfs.append(df_vip.copy().assign(Is_VIP=True).iloc[::-1])
                    if not mat_df_public.empty:
                        _dfs.append(mat_df_public.copy().assign(Is_VIP=False))
                ...
                if not mat_df.empty and 'Category' in mat_df.columns:
                        ...
                        if is_pro and st.session_state.workspace not in ["material_list", "material_overall"]:
                            with st.expander("➕ 양극재 추가"):
                                ...
                                if st.button("저장", key="btn_save_cat", use_container_width=True):
                                    try:
                                        conn = st.connection("gsheets", type=GSheetsConnection)
                                        new_data = pd.DataFrame([{"Name": n_cat, "Category": "Cathode", "Cap_Def": c_cat, "Volt_Def": v_cat, "Den_Def": 2.2}])
                                        conn.update(spreadsheet=URL_MATS, worksheet=st.session_state.workspace, data=pd.concat([df_vip, new_data], ignore_index=True).fillna(""))
                                        st.cache_data.clear()
                        ...
                        if is_pro and st.session_state.workspace not in ["material_list", "material_overall"]:
                            with st.expander("➕ 음극재 추가"):
                                ...
                                if st.button("저장", key="btn_save_ano", use_container_width=True):
                                    try:
                                        conn = st.connection("gsheets", type=GSheetsConnection)
                                        new_data = pd.DataFrame([{"Name": n_ano, "Category": "Anode", "Cap_Def": c_ano, "Volt_Def": v_ano, "Den_Def": 1.1}])
                                        conn.update(spreadsheet=URL_MATS, worksheet=st.session_state.workspace, data=pd.concat([df_vip, new_data], ignore_index=True).fillna(""))
                                        st.cache_data.clear()
                                        st.success("소재가 저장되었습니다.")
                                        st.rerun()
                        ...
                        if is_pro and st.session_state.workspace not in ["material_list", "material_overall"]:
                            with st.expander("➕ 전해액 추가"):
                                ...
                                if st.button("저장", key="btn_save_ele", use_container_width=True):
                                    try:
                                        conn = st.connection("gsheets", type=GSheetsConnection)
                                        new_data = pd.DataFrame([{"Name": n_ele, "Category": "Electrolyte", "Den_Def": d_ele}])
                                        conn.update(spreadsheet=URL_MATS, worksheet=st.session_state.workspace, data=pd.concat([df_vip, new_data], ignore_index=True).fillna(""))
                                        st.cache_data.clear()
                                        st.success("소재가 저장되었습니다.")
                                        st.rerun()
                        ...
                        if is_pro and st.session_state.workspace not in ["material_list", "material_overall"]:
                            with st.expander("➕ 분리막 추가"):
                                ...
                                if st.button("저장", key="btn_save_sep", use_container_width=True):
                                    try:
                                        conn = st.connection("gsheets", type=GSheetsConnection)
                                        new_data = pd.DataFrame([{"Name": n_sep, "Category": "Separator", "Load_Def": t_sep}])
                                        conn.update(spreadsheet=URL_MATS, worksheet=st.session_state.workspace, data=pd.concat([df_vip, new_data], ignore_index=True).fillna(""))
                                        st.cache_data.clear()
                                        st.success("소재가 저장되었습니다.")
                                        st.rerun()
                    ...
                    if is_pro and st.session_state.workspace not in ["material_list", "material_overall"]:
                        st.markdown("<div style='text-align: left; margin-top: 15px; color: #666; font-size: 14px; font-weight: bold;'>🔒 위 추가하는 소재는 귀사의 전용 데이터로만 저장되며, 철저히 보안 관리됩니다.</div><br>", unsafe_allow_html=True)
                    
                    if cat_sel in cat_list:
                    ...
                    def_den_min, def_den_max, def_den_val = 1.0, 5.0, 4.5
        ...
        with st.container(border=True):
            ...
            with c_3:
                ...
                with w1:
                    if porosity < 20.0:
                        st.error("⚠️ 공극률 부족: 전해액 침투 불량 위험!")
                with w2:
                    if v_np < 1.05:
                        st.error("⚠️ N/P Ratio 위험: 나트륨 석출 및 단락 위험!")
                    elif v_np >= 1.15:
                        st.warning("⚠️ N/P Ratio 과다: 에너지 밀도 하락 및 초기 비가역 증가!")
                with w3:
                    if show_adv and v_ec < 2.0:
                        st.error("⚠️ E/C Ratio 부족: 전해액 고갈에 따른 수명 급감 위험!")
        ...
        with st.container(border=True):
            ...
            with c_4: 
                ...
                v_te = t1.slider("Energy Density", 100, 350, 250, label_visibility="collapsed")
                v_tc = t2.slider("C-rate", 0.1, 10.0, 1.0, label_visibility="collapsed")
                v_tl = t3.slider("Cycle Goal", 500, 10000, 2000, label_visibility="collapsed")
            st.markdown("<br>", unsafe_allow_html=True)
            ...
            with c_5:
                ...
                if run_clicked:
                    ...
                    if st.session_state.history: 
                        ...
                        is_dup = all(log_data[k] == last_run.get(k) for k in keys_to_check)
                    ...
                    if is_dup:
                        ...
                        with st.spinner("🚀 물리 엔진 연산 및 시뮬레이션 진행 중..."):
                            ...
                            st.session_state.update({'sim_result': log_data, 'trigger_auto_bot': True})
                            st.rerun()
                ...
                if st.session_state.history:
                    ...
                    with g1: 
                        ...
                        st.markdown("<div style='display:flex; align-items:flex-start; color:#666; font-size:13px; margin-top:5px;'><span style='margin-right:5px;'>💡</span><span style='line-height:1.4;'>고율 방전 시 분극(Polarization) 및 IR Drop에 의한 초기 과전압(Overpotential) 크기를 나타내며, Plateau 구간의 기울기가 실가용 에너지의 품질을 결정합니다.</span></div>", unsafe_allow_html=True)
                    with g2: 
                        ...
                        st.markdown("<div style='display:flex; align-items:flex-start; color:#666; font-size:13px; margin-top:5px;'><span style='margin-right:5px;'>💡</span><span style='line-height:1.4;'>주요 상전이(Phase transition) 구간의 가역성을 진단합니다. 피크의 브로드닝(Broadening) 및 전압 Shift 현상은 활물질의 구조적 열화나 저항 증가를 암시합니다.</span></div>", unsafe_allow_html=True)
                    with g3: 
                        # 🔥 [핵심 3] Radar 네이밍 수정 🔥
                        ...
                        fig3 = go.Figure()
                        fig3.add_trace(go.Scatterpolar(r=r_vals, theta=categories, fill='toself', line=dict(color='#E4B526', width=2)))
                        ...
                        st.markdown("<div style='display:flex; align-items:flex-start; color:#666; font-size:13px; margin-top:5px;'><span style='margin-right:5px;'>💡</span><span style='line-height:1.4;'>5대 핵심 설계 지표의 Trade-off 밸런스입니다. 특정 축의 극단적 돌출 설계는 폼팩터 패키징 한계 및 양산성 병목(Bottle-neck)의 주요 원인이 됩니다.</span></div>", unsafe_allow_html=True)
...
# -----------------------------------------------------------------------------
# ✅ [수정] 자연스러운 서술형 허용 & 중요 내용만 도트 블릿 강조 프롬프트
SYSTEM_KNOWLEDGE = """
...
[응답 스타일 필수 지침]
- SIB 수석 연구원의 전문적인 브리핑 톤으로 부드럽게 서술하십시오.
- 핵심 데이터 수치, 중요한 결론, 개선 방안 등 '눈에 띄어야 하는 핵심 내용'은 반드시 도트 블릿('- ')을 사용하여 강조하십시오.
- 블릿 기호('-')와 텍스트 사이에 불필요한 공백을 주지 말고 바로 붙여서 작성하십시오. (예시: -나트륨 석출(Na-Plating) 위험이 감지됨)
"""
GREETING_MSG = "배터리 설계 전문 AI 시노봇입니다.\n\n-좌측 시뮬레이터 결과 분석\n-SIB 설계 관련 지식 질의응답\n\n궁금한 점을 자유롭게 입력해 주십시오."
...
if col_bot:
    with col_bot:
        ...
        if OpenAI is None: 
            st.error("⚠️ `openai` 라이브러리가 설치되지 않았습니다.")
            ...
            if st.session_state.trigger_auto_bot and st.session_state.sim_result:
                ...
                auto_prompt = "방금 시뮬레이션이 실행됨. 제공된 데이터를 분석하여 잘된 점, 개선점, 위험 요소를 엔지니어 관점에서 브리핑할 것. 중요한 포인트는 반드시 블릿('-')으로 강조할 것."
                ...
                with st.spinner("📊 실시간 데이터 분석 중..."):
                    try: 
                        response = client.chat.completions.create(
                            model="gpt-4o-mini", 
                            messages=[{"role": "system", "content": sys_prompt}, {"role": "user", "content": auto_prompt}]
                        )
            ...
            # 🔥 [핵심 5] 최근 대화가 맨 위로 오도록 챗봇 히스토리를 역순(Reverse) 출력 🔥
