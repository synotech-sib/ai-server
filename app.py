...
st.markdown("""
    ...
    .main-header { font-size: 26px !important; font-weight: bold !important; color: #003366; border-bottom: 2px solid #003366; padding-bottom: 10px; margin-bottom: 20px; }
...
def get_user_db(conn, url):
    """[안정화] 1000006946.jpg의 ArrowTypeError를 잡기 위한 핵심 함수"""
    try:
        df = conn.read(spreadsheet=url, worksheet="Sheet1", ttl=5)
        # [핵심] 모든 컬럼을 강제로 문자로 변환하여 타입 충돌 방지
...
# -----------------------------------------------------------------------------
if 'init_v15_pro' not in st.session_state:
    st.session_state.update({
        'logged_in': False, 'trial_count': 0, 'show_reg': False, 'reg_stage': 0,
        'v_code': "", 'temp_email': "", 'history': [], 'sim_result': None, 'init_v15_pro': True
    })
...
if st.session_state.show_reg and not st.session_state.logged_in:
    with st.container(border=True):
        ...
        if st.session_state.reg_stage == 0:
            ...
            n_comp = st.text_input("4. 회사명")
            if st.button("가입신청", disabled=not (p1==p2 and n_name)):
                try:
                    df_u = get_user_db(conn, SHEET_URL)
                    new_user = pd.DataFrame([{
                        "Email": st.session_state.temp_email, "Password": hash_password(p1), 
                        "Name": n_name, "Company": n_comp, "RegDate": datetime.now().strftime("%Y-%m-%d")
                    }])
...
with col_s2:
    if st.button("🚀 RUN DESIGN SIMULATION", use_container_width=True):
        ...
        st.session_state.sim_result = {
            ...
            "dq_x": v_axis, "dq_y": dqdv, "life": sel_mat['Life'], "time": datetime.now().strftime("%H:%M:%S")
        }
