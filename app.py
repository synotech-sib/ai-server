...
st.markdown("""
    ...
    .viewerBadge_container__1JCIV {display: none !important;} /* 우측 하단 Manage app 뱃지 숨김 */
    ...
    * { scrollbar-width: none !important; -ms-overflow-style: none !important; }
    ...
    div[data-testid="stMetricLabel"] { justify-content: center !important; }
    div[data-testid="stMetricLabel"] > div { font-weight: 800 !important; font-size: 16px !important; color: #333 !important; }
    div[data-testid="stMetricValue"] { font-size: 28px !important; color: #1A729A !important; font-weight: 900 !important; text-align: center; width: 100%;} 
    ...
    /* 🔥 S & C 로고 애니메이션 (Noto Sans KR Black, #BFBFBF) 🔥 */
    .anim-logo-container { 
        display: flex; justify-content: center; align-items: center; 
        height: 80px; padding-top: 15px; overflow: hidden;
    }
    .logo-s { 
        font-family: 'Noto Sans KR', sans-serif; font-weight: 900; font-size: 65px; 
        color: #BFBFBF; display: inline-block; 
        animation: spin-s 1.5s cubic-bezier(0.25, 1, 0.5, 1) forwards; 
    }
    .logo-c { 
        font-family: 'Noto Sans KR', sans-serif; font-weight: 900; font-size: 65px; 
        color: #BFBFBF; display: inline-block; margin-left: -18px; 
        animation: spin-c 1.5s cubic-bezier(0.25, 1, 0.5, 1) forwards; 
    }
    @keyframes spin-s {
...
# -----------------------------------------------------------------------------
# 4. 헤더 영역 (🔥 상하단 0.08 : 0.67 : 0.25 픽셀퍼펙트 일치 🔥)
# -----------------------------------------------------------------------------
...
# -----------------------------------------------------------------------------
# 5. 👑 [최고 관리자 전용 대시보드] - 화면 최상단 전체 폭 사용
# -----------------------------------------------------------------------------
if is_pro and st.session_state.get('is_admin', False):
    with st.container(border=True):
        ...
        if a1.button("👥 유저 관리 DB", use_container_width=True): 
            ...
            st.session_state.admin_ws = 'Users'; st.rerun()
        if a2.button("🔋 소재 DB", use_container_width=True): 
            ...
            st.session_state.admin_ws = 'material_overall'; st.rerun()
        if a3.button("⚙️ 파라미터 DB", use_container_width=True): 
            ...
            st.session_state.admin_ws = 'param_config'; st.rerun()
        if a4.button("💾 로그 DB", use_container_width=True): 
            ...
            st.session_state.admin_ws = 'myData'; st.rerun()
        # 관리자 챗봇 로그 확인 기능
...
# -----------------------------------------------------------------------------
# 6. 메인 바디 (✅ 0.08 : 0.67 : 0.25 레이아웃 통일)
# -----------------------------------------------------------------------------
if st.session_state.show_bot:
    ...
    with st.container(border=False):
        ...
        with st.container(border=True):
            ...
            if run_clicked:
                ...
                if is_dup:
                    st.warning("⚠️ 이전 실행과 파라미터가 동일합니다. (중복 데이터 저장 방지)")
                else:
            ...
            with st.container(border=True):
                ...
                if not db_df_all.empty and 'Email' in db_df_all.columns:
                    ...
                    if not my_saved_data.empty:
                        ...
                        if not edited_df['User Comment'].equals(df_display['User Comment']):
                            conn_u = st.connection("gsheets", type=GSheetsConnection)
                            db_u = conn_u.read(spreadsheet=URL_LOGS, worksheet="myData", ttl=600)
                            for idx, row in edited_df.iterrows():
                                mask = (db_u['Email'] == st.session_state.user_email) & (db_u.get('Workspace') == st.session_state.workspace) & (db_u['Time'] == row['Time'])
                                if mask.any(): db_u.loc[mask, 'User Comment'] = row['User Comment']
                            conn_u.update(spreadsheet=URL_LOGS, worksheet="myData", data=db_u)
                            st.cache_data.clear()
                            st.success("코멘트가 자동으로 저장되었습니다.")
                            st.rerun()
                            
                        st.caption("ℹ️ 셀 색상이 지정된 '사용자 코멘트' 칸을 더블클릭하여 내용을 입력하고 바깥 영역을 누르면 **자동으로 클라우드에 저장**됩니다.")

# -----------------------------------------------------------------------------
# (C) 우측 패널 - 🤖 시노봇 (SynoBot) AI 패널 (상단 입력칸 + 역방향)
# -----------------------------------------------------------------------------
# 🔥 시스템 프롬프트: 부드러운 톤, 도트 블릿 강조, Pro Mode 홍보 추가 🔥
SYSTEM_KNOWLEDGE = """
...
if col_bot:
    with col_bot:
        ...
        # 3. 최근 대화가 맨 위로 오는 역방향 컨테이너 (메인과 같이 늘어남)
        with st.container(border=True):
