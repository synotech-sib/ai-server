...
# ✅ 탭 이름을 Users로 바꾼 사항 적용 완료
def get_user_db():
    return load_cloud_data(URL_USERS, "Users")
        ...
        if login_btn or st.session_state.pop('trigger_login', False):
            ...
            if u_id_clean in ADMIN_USERS and u_pw == ADMIN_PW:
                ...
                if not valid.empty:
                    ...
                    vip_map = {v.lower(): v for v in vip_exact_list}
...
if st.session_state.logged_in and st.session_state.is_admin:
    with st.container(border=True):
        ...
        a4.link_button("💾 기술 로그 DB (Logs)", URL_LOGS, use_container_width=True)
        ...
        if st.session_state.reg_stage == 0:
            ...
            if st.button("가입신청", use_container_width=True, disabled=not bool(pw1 and (pw1 == pw2) and n_name and agree_terms)):
                try:
                    conn = st.connection("gsheets", type=GSheetsConnection)
                    # ✅ Users 탭으로 변경 적용
                    df_u = conn.read(spreadsheet=URL_USERS, worksheet="Users", ttl=5)
...
if st.session_state.get('show_profile') and st.session_state.logged_in:
    with st.container(border=True):
        ...
        if st.session_state.user_email in ADMIN_USERS: st.info("관리자 계정은 시트에서 직접 수정해야 합니다.")
        else:
            ...
            p1, p2 = st.columns(2); m_pw = p1.text_input("새 Password (변경 시에만)", type="password"); m_name = p2.text_input("이름", value=u_row.get('Name', ''))
            m_comp = p1.text_input("Company", value=u_row.get('Company', '')); m_dept = p2.text_input("부서", value=u_row.get('Dept', ''))
            m_job = p1.text_input("업무", value=u_row.get('Job', '')); m_phone = p2.text_input("전화", value=u_row.get('Phone', ''))
            if st.button("개인정보 수정 완료"):
                ...
                _ = conn.update(spreadsheet=URL_USERS, worksheet="Users", data=df_update)
...
with st.container(border=True):
    ...
    if not mat_df.empty:
        ...
        cat_sel = c1.selectbox("Cathode", cat_list if cat_list else ["Sample"], key="sel_cat_m")
        ano_sel = c2.selectbox("Anode", mat_df[mat_df['Category']=='Anode']['Name'].tolist(), key="sel_ano_m")
        ele_sel = c3.selectbox("Electrolyte", mat_df[mat_df['Category']=='Electrolyte']['Name'].tolist(), key="sel_ele_m")
        sep_sel = c4.selectbox("Separator", mat_df[mat_df['Category']=='Separator']['Name'].tolist(), key="sel_sep_m")
...
with st.container(border=True):
    ...
    expert = True if is_pro else st.checkbox("세부 수정 활성화 (Pro 전용)", disabled=True)
    s1, s2, s3, s4 = st.columns(4)
    v_cap = s1.slider("Capacity (mAh/g)", def_cap_min, def_cap_max, def_cap_val)
    v_volt = s2.slider("Voltage (V)", def_vlt_min, def_vlt_max, def_vlt_val)
    v_dens = s3.slider("Density (g/cc)", def_den_min, def_den_max, def_den_val, disabled=not expert)
    v_life = s4.slider("Base Life (Cycles)", def_lif_min, def_lif_max, def_lif_val, disabled=not expert)

with st.container(border=True):
    st.markdown('<p class="main-header">3. Process Parameters</p>', unsafe_allow_html=True)
    adv = True if is_pro else st.checkbox("상세 파라미터 수정 활성화 (Pro 전용)", disabled=True)
    p1, p2, p3 = st.columns(3)
    v_load = p1.slider("Loading (mg/cm2)", def_lod_min, def_lod_max, def_lod_val)
    p1.slider("Cathode Press Density", 1.5, 3.5, 2.5, disabled=not adv)
    p1.slider("Conductive Agent %", get_p('cat_conductive', 'Min', 0.5), get_p('cat_conductive', 'Max', 10.0), get_p('cat_conductive', 'Default', 2.0), disabled=not adv)
    p1.slider("Binder %", get_p('cat_binder', 'Min', 0.5), get_p('cat_binder', 'Max', 10.0), get_p('cat_binder', 'Default', 3.0), disabled=not adv)
    v_np = p2.slider("N/P Ratio", 1.0, 1.5, 1.15, step=0.01)
    p2.slider("Anode Press Density", 0.8, 2.0, 1.1, disabled=not adv)
    p2.slider("Anode Active %", 80.0, 98.0, 95.0, disabled=not adv)
    v_act = p3.slider("Active Ratio (%)", 80.0, 99.0, 92.0)
    p3.slider("E/C Ratio (g/Ah)", get_p('ec_ratio', 'Min', 1.0), get_p('ec_ratio', 'Max', 8.0), get_p('ec_ratio', 'Default', 3.5), disabled=not adv)
    p3.slider("Separator Thick (μm)", int(get_p('sep_thick', 'Min', 5)), int(get_p('sep_thick', 'Max', 50)), int(get_p('sep_thick', 'Default', 16)), disabled=not adv)

with st.container(border=True):
    st.markdown('<p class="main-header">4. Target Settings</p>', unsafe_allow_html=True)
    t1, t2 = st.columns(2)
    v_te = t1.slider("Energy Goal (Wh/kg)", 100, 250, 160)
    v_tc = t2.slider("Simulation C-rate", get_p('target_crate', 'Min', 0.1), get_p('target_crate', 'Max', 10.0), get_p('target_crate', 'Default', 1.0))
...
with st.container(border=True):
    ...
    if st.button("🚀 RUN DESIGN SIMULATION", use_container_width=True):
        ir_drop = 0.1 + (v_tc * 0.02); cell_v = max(0.1, v_volt - ir_drop); res_whkg = ((v_cap * (v_act/100) * cell_v) / 2.5) * max(0.5, 1.0 - (v_tc * 0.015))
        log_data = {"Time": (datetime.utcnow() + timedelta(hours=9)).strftime("%H:%M:%S"), "Cathode": cat_sel, "Anode": ano_sel, "Cap(mAh/g)": round(v_cap, 1), "Volt(V)": round(v_volt, 2), "Load(mg)": round(v_load, 1), "N/P Ratio": v_np, "Active(%)": v_act, "C-rate": v_tc, "Wh/kg": round(res_whkg, 1), "Cell_V": round(cell_v, 2), "Life(Cyc)": int(v_life * (0.95 ** v_tc))}
        v_axis, dqdv = get_dqdv(cat_sel, v_tc, mat_df); log_data.update({"dq_x": v_axis, "dq_y": dqdv})
    ...
    if st.session_state.history:
        ...
        res = st.session_state.history[st.selectbox("기록 선택", range(len(log_opts)), format_func=lambda x: log_opts[x])]
        st.markdown("---")
...
if is_pro and st.session_state.history:
    with st.container(border=True):
        ...
        c1, c2 = st.columns(2)
        if c1.button("💾 내 계정에 저장하기", use_container_width=True):
            try:
                conn = st.connection("gsheets", type=GSheetsConnection); db_df = conn.read(spreadsheet=URL_LOGS, worksheet="myData", ttl=0)
                if db_df[(db_df['Email'] == st.session_state.user_email) & (db_df['Time'] == res['Time']) & (db_df.get('Workspace', 'material_list') == st.session_state.workspace)].empty:
                    save_rec = res.copy(); save_rec.update({'Email': st.session_state.user_email, 'Workspace': st.session_state.workspace})
                    ...
                    st.success("저장 완료!")
                else: st.warning("이미 저장된 기록입니다.")
            except Exception as e: st.error(f"저장 실패: {e}")

        if c2.button("🗑️ 현재 기록 삭제", use_container_width=True):
            try:
                conn = st.connection("gsheets", type=GSheetsConnection); db_df = conn.read(spreadsheet=URL_LOGS, worksheet="myData", ttl=0)
                _ = conn.update(spreadsheet=URL_LOGS, worksheet="myData", data=db_df[~((db_df['Email'] == st.session_state.user_email) & (db_df['Time'] == res['Time']) & (db_df.get('Workspace', 'material_list') == st.session_state.workspace))])
        ...
        df_exp = pd.DataFrame(st.session_state.history).drop(columns=['dq_x', 'dq_y'], errors='ignore')
        buf = io.BytesIO()
        with pd.ExcelWriter(buf, engine='openpyxl') as wr: df_exp.to_excel(wr, index=False)
        r2c1.download_button("📥 엑셀 다운로드", buf.getvalue(), f"SynoCore_{st.session_state.workspace}.xlsx", use_container_width=True)
        if FPDF:
            r2c2.download_button("📄 선택 PDF", create_pdf([res], "Result"), f"Result_{res['Time']}.pdf", use_container_width=True)
            r2c3.download_button("📑 전체 PDF", create_pdf(st.session_state.history, "All Logs"), "All_Logs.pdf", use_container_width=True)
