...
# [2] Material Specs Expert Mode (자물쇠 적용)
with st.container(border=True):
    st.markdown('<p class="main-header">2. Material Specs Expert Mode</p>', unsafe_allow_html=True)
    
    # [자물쇠 기능] 로그인 여부에 따라 텍스트 변경 및 활성화 제어
    exp_label = "🔓 물성 직접 수정 활성화" + (" :red[(Pro Mode 전용)]" if not st.session_state.logged_in else "")
...
with st.container(border=True):
    ...
    # [자물쇠 기능] 로그인 여부에 따라 텍스트 변경 및 활성화 제어
    adv_label = "🔍 더 자세히 보기 (Advanced Settings)" + (" :red[(Pro Mode 전용)]" if not st.session_state.logged_in else "")
...
with st.container(border=True):
    ...
    if st.button("🚀 RUN DESIGN SIMULATION", key="btn_run_m"):
        if st.session_state.trial_count < 10 or st.session_state.logged_in:
            ...
            log_data = {"Time": cur_time, "Cathode": cat_sel, "Anode": ano_sel, "Cap(mAh/g)": v_cap, "Volt(V)": v_volt, "Load(mg)": v_load, "N/P Ratio": v_np, "Active(%)": v_act, "C-rate": v_tc, "Wh/kg": round(res_whkg, 1), "Cell_V": round(cell_v, 2), "Life(Cyc)": v_life}
    ...
    if st.session_state.sim_result:
        ...
        with g2:
            ...
            st.table(pd.DataFrame({"Parameter": ["Cathode", "Anode", "Loading", "N/P Ratio", "Test C-rate"], "Value": [res['Cathode'], res['Anode'], f"{res['Load(mg)']} mg/cm2", res['N/P Ratio'], f"{res['C-rate']} C"]}))
