...
# 구글 시트 라이브러리 예외 처리 (ModuleNotFoundError 방지)
try:
...
# -----------------------------------------------------------------------------
# 2. [에러 해결] 세션 상태 초기화 (AttributeError 원천 차단)
# -----------------------------------------------------------------------------
...
# -----------------------------------------------------------------------------
# 3. [데이터 로드] 엑셀 및 구글 시트 안전 연결 (ValueError 극복)
# -----------------------------------------------------------------------------
...
with h_r:
    if not st.session_state.logged_in:
        ...
        u_id = l_c1.text_input("ID", placeholder="email", key="id_login", label_visibility="collapsed")
        u_pw = l_c2.text_input("PW", type="password", placeholder="password", key="pw_login", label_visibility="collapsed")
        ...
        with c1:
            if st.button("계정생성 ㅣ Pro 회원가입", key="btn_go_reg"): st.session_state.show_reg = not st.session_state.show_reg
        with c2:
...
if st.session_state.show_reg and not st.session_state.logged_in:
    with st.container(border=True):
        ...
        if st.session_state.reg_stage == 0:
            e_in = st.text_input("1. 회사 이메일 주소", key="r_email")
            ...
            p1, p2 = st.columns(2)
            pw1 = p1.text_input("2. Password", type="password", key="r_p1")
            ...
            agree = st.checkbox("참조용 자료이며 책임지지 않음에 동의", key="r_a")
            if st.button("가입신청", disabled=not (agree and pw1==pw2 and n_name), key="r_fin"):
                try:
                    ...
                    st.error("⚠️ 구글 시트 저장 불가: 서비스 계정(JSON) 인증 필요. (읽기는 정상 작동)")

st.markdown("<br>", unsafe_allow_html=True)

# -----------------------------------------------------------------------------
# 5. 시뮬레이터 본문 (슬라이더 및 데이터 완전 복구)
# -----------------------------------------------------------------------------
...
with st.container(border=True):
    ...
    if not mat_df.empty:
        ...
        cat_sel = m1.selectbox("Cathode", mat_df[mat_df['Category']=='Cathode']['Name'].tolist(), key="sel_cat")
        ...
        ano_sel = m2.selectbox("Anode", ["Hard Carbon (A)", "Hard Carbon (B)"], key="sel_ano")
        m3.selectbox("Electrolyte", ["Standard NaPF6"], key="sel_ele")
        m4.selectbox("Separator", ["PE 16um"], key="sel_sep")
    else:
        st.warning("material_list.xlsx 없음")
...
with st.container(border=True):
    ...
    if expert:
        v_cap = s1.slider("Capacity (mAh/g)", 100.0, 220.0, c_cap_i, key="sl_cap")
        v_volt = s2.slider("Voltage (V)", 2.5, 4.5, c_volt_i, key="sl_volt")
        v_dens = s3.slider("Density (g/cc)", 1.5, 4.0, c_dens_i, key="sl_dens")
        v_life = s4.slider("Base Life (Cycles)", 500, 10000, c_life_i, key="sl_life")
    else:
...
# [3] Process Parameters (7개 펼침 노출 완료)
with st.container(border=True):
    ...
    with p1:
        ...
        v_load = st.slider("Loading (mg/cm2)", 5.0, 45.0, c_load_i, key="sl_load")
        if show_adv:
            st.slider("Cathode Press Density", 1.5, 3.5, 2.5, key="ad_c_den")
            st.slider("Conductive Agent %", 0.5, 5.0, 2.0, key="ad_c_con")
            st.slider("Binder %", 0.5, 5.0, 3.0, key="ad_c_bin")
    with p2:
        st.markdown('<p class="sub-header-bold">(B) Anode & Balance</p>', unsafe_allow_html=True)
        v_np = st.slider("N/P Ratio", 1.0, 1.5, 1.15, key="sl_np")
        if show_adv:
            st.slider("Anode Press Density", 0.8, 2.0, 1.1, key="ad_a_den")
            st.slider("Anode Active %", 90.0, 98.0, 95.0, key="ad_a_act")
    with p3:
        st.markdown('<p class="sub-header-bold">(C) Cell</p>', unsafe_allow_html=True)
        v_act = st.slider("Active Ratio (%)", 80.0, 99.0, 92.0, key="sl_act")
        if show_adv:
            st.slider("E/C Ratio (g/Ah)", 1.0, 8.0, 3.5, key="ad_ec")
            st.slider("Separator Thick (μm)", 12, 30, 16, key="ad_sep")
    st.markdown("<br>", unsafe_allow_html=True)
...
with st.container(border=True):
    ...
    t1, t2 = st.columns(2)
    v_te = t1.slider("Energy Goal (Wh/kg)", 100, 250, 160, key="sl_te")
    v_tc = t2.slider("Simulation C-rate", 0.1, 20.0, 1.0, key="sl_tc")
    ...
    if st.session_state.history:
        ...
        sel_idx = st.selectbox("기록 선택", range(len(log_opts)), format_func=lambda x: log_opts[x], key="sel_hist")
    ...
    if st.session_state.sim_result:
        ...
        with g1:
            ...
            # [에러 해결] DuplicateElementId 완벽 방지를 위한 랜덤 키 결합
            st.plotly_chart(fig, use_container_width=True, key=f"plot_{res['Time']}_{random.randint(1,10000)}")
        with g2:
    ...
    # [요청 반영] 12가지 상세 데이터가 담긴 로그 전체 테이블 출력
    if st.session_state.history:
        st.markdown("---")
