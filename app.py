...
def get_user_db():
    if conn:
        try:
            # 시트 탭 이름이 Sheet1인지 확인 필요 (다르면 수정)
...
if st.session_state.show_reg and not st.session_state.logged_in:
    with st.container(border=True):
        ...
        if st.session_state.reg_stage == 0:
            ...
            if st.button("인증번호 6자리 발송", key="r_v_send"):
            ...
            v_in = st.text_input("인증번호 입력", key="r_v_input")
            ...
            n_comp = st.text_input("4. Company (회사명)", key="r_comp")
            n_dept = st.text_input("5. 부서", key="r_dept")
            n_job = st.text_input("6. 담당업무", key="r_job")
            n_phone = st.text_input("7. 연락처 (전화번호)", key="r_phone")
            
            with st.expander("(동의) 내용보기"):
                st.markdown("참조용 자료이며 실제 결과에 대해 **시노코어는 책임을 지지 않는다.**")
            agree = st.checkbox("약관에 동의합니다.", key="r_agree")
            
            can_sub = agree and (pw1 == pw2) and pw1 and n_name and n_comp
            if st.button("가입신청", disabled=not can_sub, key="r_btn_final"):
                try:
                    df_u = get_user_db()
                    ...
                    st.success("신청 완료! 관리자 승인 후 로그인 가능합니다."); st.session_state.show_reg = False; st.session_state.reg_stage = 0
                except Exception as e:
                    st.error(f"DB 저장 오류: 구글 시트의 공유 설정(편집자)을 확인하세요. ({e})")
...
with st.container(border=True):
    ...
    if not mat_df.empty:
        ...
        # 자동 연동 초기값
        c_cap_init = float(sel_row.get('Capacity', 160))
        c_volt_init = float(sel_row.get('Voltage', 3.05))
        c_dens_init = float(sel_row.get('Density', 2.2))
        c_life_init = int(sel_row.get('Life', 4000))
        c_load_init = float(sel_row.get('Rec_Loading', 14.0))
...
# [2] Material Specs (4개 슬라이더 모두 활성화)
with st.container(border=True):
    st.markdown('<p class="main-header">2. Material Specs Expert Mode</p>', unsafe_allow_html=True)
    expert = st.checkbox("🔓 물성 직접 수정 활성화", key="chk_expert")
    s1, s2, s3, s4 = st.columns(4)
    if expert:
        v_cap = s1.slider("Capacity (mAh/g)", 100.0, 220.0, c_cap_init, key="sld_cap")
        v_volt = s2.slider("Voltage (V)", 2.5, 4.5, c_volt_init, key="sld_volt")
        v_dens = s3.slider("Density (g/cc)", 1.5, 4.0, c_dens_init, key="sld_dens")
        v_life = s4.slider("Base Life (Cycles)", 500, 10000, c_life_init, key="sld_life")
    else:
        v_cap, v_volt, v_dens, v_life = c_cap_init, c_volt_init, c_dens_init, c_life_init
...
with st.container(border=True):
    ...
    with p1:
        ...
        v_load = st.slider("Loading (mg/cm2)", 5.0, 45.0, c_load_init, key="sld_load")
        if show_adv:
            st.slider("Cathode Press Density", 1.5, 3.5, 2.5, key="adv_c_dens")
            st.slider("Conductive Agent %", 0.5, 5.0, 2.0, key="adv_c_cond")
            st.slider("Binder %", 0.5, 5.0, 3.0, key="adv_c_bind")
    with p2:
    ...
    with p3:
        ...
        if show_adv:
            ...
            st.slider("Separator Thick (μm)", 12, 30, 16, key="adv_sep")
    st.markdown("<br>", unsafe_allow_html=True)
...
with st.container(border=True):
    ...
    if st.session_state.sim_result:
        ...
        with g1:
            ...
            st.plotly_chart(fig, use_container_width=True, key="fig_res")
        with g2:
