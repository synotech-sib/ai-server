...
st.markdown("""
    ...
    * { scrollbar-width: none !important; -ms-overflow-style: none !important; }
    ...
    td[data-testid="stTableCell"]:nth-child(3) { background-color: #F3F0F4 !important; color: black !important; font-weight: 500; }
    ...
    .logo-c { 
        font-family: 'Noto Sans KR', sans-serif; font-weight: 900; font-size: 65px; 
        color: #BFBFBF; display: inline-block; margin-left: -15px; 
...
# -----------------------------------------------------------------------------
# 4. 헤더 영역 (🔥 상/하단 0.1 : 0.7 : 0.2 로 완벽한 폭 일치 🔥)
# -----------------------------------------------------------------------------
# 시노봇 활성화 여부에 따라 3열 또는 2열 배치
if st.session_state.show_bot:
    h_col1, h_col2, h_col3 = st.columns([0.1, 0.7, 0.2], gap="large")
else:
    h_col1, h_col2 = st.columns([0.1, 0.9], gap="large")
    h_col3 = None

with h_col1:
    # 좌측 패널 상단: 로고 애니메이션 출력
    st.markdown("""
...
with h_col2:
    # 메인 패널 상단: 타이틀과 로그인 팝오버를 분할하여 배치
    hc_title, hc_login = st.columns([0.8, 0.2])
    with hc_title:
    ...
    with hc_login:
        if not is_pro:
            ...
            st.markdown(f'<div class="user-greeting" style="justify-content:center;">{st.session_state.user_name} ({level_tag})</div>', unsafe_allow_html=True)

# 우측 패널 상단: 계정가입 버튼과 챗봇 토글 (시노봇 박스와 가로폭이 100% 동일함)
target_h_right = h_col3 if st.session_state.show_bot else hc_login
...
with target_h_right:
    ...
    # 시노봇 토글
    st.toggle("**💬 SynoBot 활성화**", value=st.session_state.show_bot, key="bot_toggle_ui")
    ...
    if st.session_state.admin_view is not None or st.session_state.show_profile is False:
        with st.container(border=True):
            ...
            if a4.button("💾 로그 DB", use_container_width=True):
                ...
                st.rerun()
            if a5.button("🤖 챗봇 로그 확인", use_container_width=True):
...
# (B) 중앙 패널 - 유저 폼 및 시뮬레이터 (메인 폭 70% 안에서만 렌더링)
with col_main:
    # 계정 가입 폼
            ...
            with st.container(border=True):
                ...
                if not db_df_all.empty and 'Email' in db_df_all.columns:
                    ...
                    if not my_saved_data.empty:
                        ...
                        # 🔥 사용자 코멘트 더블클릭 "자동 저장" 로직 (수동 저장 버튼 삭제) 🔥
                        ...
                        st.caption("ℹ️ 셀 색상이 지정된 '사용자 코멘트' 칸을 더블클릭하여 내용을 수정하고 바깥 영역을 클릭하면 **자동으로 클라우드에 저장**됩니다.")
...
4. If a free user asks a general question, casually mention at the end: "\n\n💡 팁: Pro Mode로 가입 또는 전환하시면 귀사만의 단독 데이터 보안 관리 및 맞춤형 고급 시뮬레이션 정보를 제공받으실 수 있습니다."
"""
